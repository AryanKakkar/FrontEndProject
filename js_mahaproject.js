...
async function getSongs(folder){ 
    let a=await fetch(`http://127.0.0.1:3000/FrontEnd/JS/${folder}/`)
...
async function main(folder) {   
    ...
    Array.from(document.querySelector(".scroll1").getElementsByTagName("li")).forEach(e=>{
        ...
        e.addEventListener("click",element=>{
            ...
            let c=`http://127.0.0.1:3000/FrontEnd/JS/${folder}/${e.querySelector(".song_name").innerHTML}-${e.querySelector(".artist").innerHTML}.mp3`
            playMusic(c,d)
        })
    })
